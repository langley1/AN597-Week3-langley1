---
title: "AN597-Week3-Hakalar2000-PeerCommentary"
author: "Me"
...
```
So ppois is used for a range of data where dpois is used to plot individual numbers. Since we don't have a set end to the number of calls that we can hear, we don't actually know the upper limit for how many calls the monkeys can make. We only know that the poisson distribution cannot go below 0. So the ppois funtion will account for a range of numbers which can be specified with a starting number q. The tails basically tell R to calculated q>= or q<=. the default for tails is always TRUE which specifies the lower tail or q<=. From my understanding you want to have the q as the starting number (in this case 8), and the lower.tail = FALSE.
The probability should be high since its 8+ and incorporates the average. So basicall 8-15 calls and 15-22 calls should have a high probability of occuring. 
...
```
Same dude. I just used rpois in the end. 
...
Yes same. I am confused by cumulative probability mostly and what its uses are. 
